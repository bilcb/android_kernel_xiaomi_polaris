			 ...
			 unsigned int *tlimit)
{
	 ...
	 *    A corrupted image could leave ofs such that the 4-byte
	 *    entry straddles a block boundary: refuse it instead of
	 *    reading out of bounds (valid images never straddle,
	 *    entries are 4-byte aligned and blocks are 4K multiples).
	 */
	if (it->ofs + sizeof(struct erofs_xattr_entry) > EROFS_BLKSIZ) {
		DBG_BUGON(1);
		return -EFSCORRUPTED;
	}
