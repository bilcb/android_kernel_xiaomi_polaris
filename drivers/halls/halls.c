...
static int halls_configure_irq(struct halls_data *hdata, int gpio, const char *label) {
	...
	if (irq_set_irq_wake(irq, 1))
		pr_warn("%s: failed to enable wake for GPIO %d IRQ\n",
			__func__, gpio);
...
free_irqs:
	...
	irq_set_irq_wake(gpio_to_irq(hdata->second_gpio), 0);
	free_irq(gpio_to_irq(hdata->second_gpio), hdata);
	gpio_free(hdata->second_gpio);
free_first_irq:
	irq_set_irq_wake(gpio_to_irq(hdata->first_gpio), 0);
	free_irq(gpio_to_irq(hdata->first_gpio), hdata);
