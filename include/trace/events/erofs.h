...
TRACE_EVENT(erofs_lookup,
	...
	TP_STRUCT__entry(
		...
		__dynamic_array(char,	name, NAME_MAX + 1)
	...
	TP_fast_assign(
		...
		/* copy the name: dentry may be freed before the
		 * ring buffer is read, and d_name is not NUL-terminated */
		__entry->namelen = min(dentry->d_name.len,
				       (unsigned int)NAME_MAX);
		memcpy(__get_dynamic_array(name), dentry->d_name.name,
		       __entry->namelen);
		((char *)__get_dynamic_array(name))[__entry->namelen] = '\0';
		__entry->flags	= flags;
	),
